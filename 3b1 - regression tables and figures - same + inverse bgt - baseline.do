...
* This do-file implements the baseline specifications: Table 1 (columns 1 and 3) and Figures 1A and 1C

/*----------------------------------------------------------------------------*/
*STEPS:
	*0. Set directories.
	*1. Implement the empirical specification (baseline, same + inverse samples) and generate Table 1 and Figures 1A-1C.
/*-----------------------------------------------------------------------------*/
...
global key_vars			 "${dep_var} ${unique_unit_id} ${calendar_period} ${unit_treat_date1} ${controls} ${se_cluster} cz_code_2000 soc_4d treated"
...
local post_treat  "Same-industry"
...
outreg2 using "$tables/Table 1.`fmt_tab'", ///
	...
	ctitle(`var_lab',`post_treat') nonotes
...
graph export "$figures/Figure 1A.`fmt_pic'", replace
...
event_plot, default_look ///
	...
	legend(label(1 "Pre-trend coefficients") label(2 "Treatment effects") rows(1) size(small) position(6)) ///
	text(`label_att_v' `label_att_h' "ATT = `att' (`ste')", placement(c) justification(center) size(medsmall) box fcolor(none) lcolor(gray) lwidth(medium) margin(medsmall))) 
graph export "$figures/Figure 1C.`fmt_pic'", replace
