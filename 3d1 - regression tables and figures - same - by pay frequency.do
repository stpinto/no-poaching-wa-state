...
* This do-file implements the specifications that split the sample between hourly wage and annual salary workers (same-industry sample, Figures A1A-A1C)

/*----------------------------------------------------------------------------*/
*STEPS:
	*0. Set directories.
	*1. Implement the empirical specification that splits the sample by payfrequency and generate Figures A1A-A1C.
/*-----------------------------------------------------------------------------*/
...
* i2) Event study regression and plot - hourly pay
...
graph export "$figures/Figure A1A.`fmt_pic'", replace

restore

	
	
* j1) DiD Regression - annual pay
preserve
...
event_plot, default_look ///
	...
	legend(label(1 "Pre-trend coefficients") label(2 "Treatment effects") rows(1) size(small) position(6)) ///
	text(`label_att_v' `label_att_h' "ATT = `att' (`ste')", placement(c) justification(center) size(medsmall) box fcolor(none) lcolor(gray) lwidth(medium) margin(medsmall))) 
graph export "$figures/Figure A1C.`fmt_pic'", replace
restore
