...
* This do-file creates Figure B1, panel A

/*----------------------------------------------------------------------------*/
*STEPS:
	*0. Set directories.
	*1. Convert the csv file from BGT with Indeed and Linkedin ads into dta.
	*1. Copmutes the number of observations, with and without wage estimates, for the same-industry sample.
/*-----------------------------------------------------------------------------*/
...
*------------------------------------------------------------------------------*
* 1) Convert Lightcast's csv file into dta
*------------------------------------------------------------------------------*
import delimited "${data_bgt_r}/IDs with Indeed or LinkedIn as Source, 2010-2022.csv"
drop 	v1 
rename 	id bgtjobid
format 	bgtjobid %15.0f
compress
save 			 "${data_bgt_c}/IDs with Indeed or LinkedIn as Source, 2010-2022.dta", replace




*------------------------------------------------------------------------------*
* 2) Generate graphs with number of observations with + without estimates -- same-industry sample
*------------------------------------------------------------------------------*
...
* j) Collapse + Save: temp file
preserve
gen obs = 1 
collapse (sum) obs if sample == 1, by(yq)

rename obs obs_all

tempfile obs_by_yq_all
save 	`obs_by_yq_all'
restore



* k) Merge: file with estimates
// Tag and drop estimated wages
...
* l) Collapse + Merge: temp file
gen 		   obs = 1 
collapse (sum) obs if sample == 1 & cannot_impute == 0, by(yq)
rename 		   obs obs_no_est

merge 1:1 yq using `obs_by_yq_all'
keep if _merge == 3


foreach var in obs_all obs_no_est {
...
* m) Generate: graph
twoway (connected obs_all yq) (connected obs_no_est yq), ///
	ytitle(Number of observations (million)) ylabel(, format(%9.1f)) ///
	xtitle("") xlabel(228(2)247) ///
	legend(order(1 "All wages" 2 "Non-estimated wages")  rows(1) size(small) position(6))
graph export "$figures/Figure B1A.`fmt_pic'", replace
