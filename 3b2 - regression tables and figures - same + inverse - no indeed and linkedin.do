...
* This do-file implements the specifications that exclude the job ads coming from Indeed and LinkedIn

/*----------------------------------------------------------------------------*/
*STEPS:
	*0. Set directories.
	*1. Exclude Indeed/Linkedin job ads and re-estimate the baseline specifications (generate Table B1 and Figures B1B-B1D).
/*-----------------------------------------------------------------------------*/
...
global key_vars			"bgtjobid ${dep_var} ${unique_unit_id} ${calendar_period} ${unit_treat_date1} ${se_cluster} cz_code_2000 soc_4d treated"
...
drop 	franchise settlement_date
compress
...
local sample_0    "Same-ind. sample"
local sample_1    "Baseline"
local sample_2    "All observations"
...
outreg2 using "$tables/Table B1.`fmt_tab'", ///
	bdec(`decimals') sdec(`decimals') adec(`decimals') nocons tex(frag) label replace `asterisk' sortvar(pre5 pre4 pre3 pre2 pre1) ///
	addtext(Year-quarter x CZ FEs, Y, Year-quarter x SOC-4d FEs, Y, SOC-4d x Employer FEs, Y) ///
	addstat(Pre-treatment F-stat,`e(pre_F)',Pre-treatment p-value,`e(pre_p)') ///
	ctitle(`var_lab',`sample_0',`sample_1',`sample_2') nonotes

	

*** ii) NO IMPUTATION ESTIMATES - AFTER REMOVAL OF INDEED/LINKEDIN LIST

* j) Tag and drop estimated wages
merge 1:1 bgtjobid using "${data_bgt_c}/IDs with Indeed or LinkedIn as Source, 2010-2022"
...
* k1) DiD Regression
local var_lab: 	  var label $dep_var
local sample_1    "Excluding all"
local sample_2    "Indeed-Linkedin"
...
did_imputation $dep_var $unique_unit_id $calendar_period $unit_treat_date1 , fe($fixed_effects) cluster($se_cluster) $other_specs_es
outreg2 using "$tables/Table B1.`fmt_tab'", ///
	bdec(`decimals') sdec(`decimals') adec(`decimals') nocons tex(frag) label append `asterisk' sortvar(pre5 pre4 pre3 pre2 pre1) ///
	addtext(Year-quarter x CZ FEs, Y, Year-quarter x SOC-4d FEs, Y, SOC-4d x Employer FEs, Y) ///
	addstat(Pre-treatment F-stat,`e(pre_F)',Pre-treatment p-value,`e(pre_p)') ///
	ctitle(`var_lab',`sample_0',`sample_1',`sample_2') nonotes
...
graph export "$figures/Figure B2B.`fmt_pic'", replace
...
* n) Drop: observations to allow interaction FEs to run
// Drop missing soc_4d
drop if soc_4d == .	
...
capture noisily did_imputation $dep_var $unique_unit_id $calendar_period $unit_treat_date1 , fe($fixed_effects)  controls($controls) $other_specs_did nose
...
* o2) Event study regression and plot
did_imputation $dep_var $unique_unit_id $calendar_period $unit_treat_date1 , fe($fixed_effects) cluster($se_cluster) $other_specs_es
outreg2 using "$tables/Table B1.`fmt_tab'", ///
	bdec(`decimals') sdec(`decimals') adec(`decimals') nocons tex(frag) label append `asterisk' sortvar(pre5 pre4 pre3 pre2 pre1) ///
	addtext(Year-quarter x CZ FEs, Y, Year-quarter x SOC-4d FEs, Y, SOC-4d x Employer FEs, Y) ///
...
* p) Tag and drop estimated wages
merge 1:1 bgtjobid using "${data_bgt_c}/IDs with Indeed or LinkedIn as Source, 2010-2022"	
drop if _merge == 2 | _merge == 3
...
* q2) Event study regression and plot
did_imputation $dep_var $unique_unit_id $calendar_period $unit_treat_date1 , fe($fixed_effects) cluster($se_cluster) $other_specs_es
outreg2 using "$tables/Table B1.`fmt_tab'", ///
	bdec(`decimals') sdec(`decimals') adec(`decimals') nocons tex(frag) label append `asterisk' sortvar(pre5 pre4 pre3 pre2 pre1) ///
	addtext(Year-quarter x CZ FEs, Y, Year-quarter x SOC-4d FEs, Y, SOC-4d x Employer FEs, Y) ///
...
event_plot, default_look ///
	...
	legend(label(1 "Pre-trend coefficients") label(2 "Treatment effects") rows(1) size(small) position(6)) ///
	text(`label_att_v' `label_att_h' "ATT = `att' (`ste')", placement(c) justification(center) size(medsmall) box fcolor(none) lcolor(gray) lwidth(medium) margin(medsmall))) 
graph export "$figures/Figure B2D.`fmt_pic'", replace
