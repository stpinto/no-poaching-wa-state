...
* This do-file implements the specifications that excludes non-restaurant treated chains

/*----------------------------------------------------------------------------*/
*STEPS:
	*0. Set directories.
	*1. Exclude non-restaurant treated chains and re-estimates the baseline specifications (generate Figure E1A).
/*-----------------------------------------------------------------------------*/
...
global key_vars			 "${dep_var} ${unique_unit_id} ${calendar_period} ${unit_treat_date1} ${se_cluster} cz_code_2000 soc_4d treated"
...
* g) Merge: with county-to-CZ crosswalk + list of restaurant chains
// Crosswalk
...
merge m:1 franchise_id using "${data_mat_c}/BGT - list of treated chains in restaurant industry.dta"
gen 	  restaurant_chain = (_merge == 3)
codebook  franchise_id if restaurant_chain == 1	
drop _merge 
...
* h) Restrict: BGT sample to ads from industries with no treated chains + generate: relative time period
// Merge
...
// Drop non-restaurant treatment group
drop if franchise_id != . & restaurant_chain == 0 & treated == 1	
...
* j2) Event study regression and plot
...
graph export "$figures/Figure E1A.`fmt_pic'", replace
