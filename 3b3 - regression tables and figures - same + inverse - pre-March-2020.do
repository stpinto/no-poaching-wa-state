...
* This do-file implements the specifications that exclude job ads from the post-March 2020 period

/*----------------------------------------------------------------------------*/
*STEPS:
	*0. Set directories.
	*1. Exclude post-March 2020 job ads and re-estimate the baseline specifications (generate Figures C1A-C1C).
/*-----------------------------------------------------------------------------*/
...
graph export "$figures/Figure C1A.`fmt_pic'", replace
...
event_plot, default_look ///
	...
	legend(label(1 "Pre-trend coefficients") label(2 "Treatment effects") rows(1) size(small) position(6)) ///
	text(`label_att_v' `label_att_h' "ATT = `att' (`ste')", placement(c) justification(center) size(medsmall) box fcolor(none) lcolor(gray) lwidth(medium) margin(medsmall))) 
graph export "$figures/Figure C1C.`fmt_pic'", replace
